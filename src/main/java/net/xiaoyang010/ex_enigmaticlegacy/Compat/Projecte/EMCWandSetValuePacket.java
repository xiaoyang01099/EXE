        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (!player.hasPermissions(2) && !EMCWandHelper.isAuthorized(player.getUUID())) {
                player.displayClientMessage(
                        new TranslatableComponent("message.ex_enigmaticlegacy.emc_wand.no_permission"), false);
                return;
            }
            ...
            if (success) {
                player.displayClientMessage(
                        new TranslatableComponent("message.ex_enigmaticlegacy.emc_wand.set_success",
