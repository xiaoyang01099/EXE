...
public class Crissaegrim extends SwordItem {
    ...
    public Crissaegrim() {
        super(EXEAPI.MIRACLE_ITEM_TIER, INFINITE_DAMAGE, INFINITE_ATTACK_SPEED, new Properties());
    }
    ...
    private void forceKill(LivingEntity target, Player attacker) {
        ...
        target.hurt(attacker.damageSources().playerAttack(attacker), Float.MAX_VALUE);
    ...
    private void forceDamage(LivingEntity target, Player attacker, float damage) {
        ...
        if (newHealth <= 0) {
            target.die(target.damageSources().playerAttack(attacker));
        }
