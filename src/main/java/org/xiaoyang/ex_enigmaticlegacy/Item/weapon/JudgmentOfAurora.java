...
public class JudgmentOfAurora extends ManasteelSwordItem {
        ...
        if (!world.isClientSide) {
            ...
            for (int circle = 0; circle < circles; circle++) {
                ...
                for (int i = 0; i < pixiesPerCircle; i++) {
                    ...
                    PixieEntity pixie = new PixieEntity(world);
                    pixie.setPos(
                            player.getX() + offsetX,
     ...
     * 智能分配目标给精灵
     */
    private LivingEntity assignSmartTarget(List<LivingEntity> availableTargets, PixieEntity pixie, int circle, int pixieIndex) {
                    ...
                    FallingStarEntity star = new FallingStarEntity(player, world) {
                        ...
                        protected void onHitEntity(@Nonnull EntityHitResult hit) {
                            ...
                            if (e != getOwner() && e.isAlive()) {
                                e.hurt(e.damageSources().playerAttack((Player) getOwner()), STAR_DAMAGE);
                            }
                            discard();
                        }
                    };
