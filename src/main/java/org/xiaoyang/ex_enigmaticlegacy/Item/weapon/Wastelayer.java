...
import org.xiaoyang.ex_enigmaticlegacy.Client.particle.ef.Effect;
import org.xiaoyang.ex_enigmaticlegacy.Client.particle.ef.EffectCut;
import org.xiaoyang.ex_enigmaticlegacy.Client.particle.ef.EffectSlash;
...
import org.xiaoyang.ex_enigmaticlegacy.Network.inputMessage.EffectMessage;
...
public class Wastelayer extends SwordItem {
            ...
            if (canUseManaBlade(stack)) {
                if (player.getCooldowns().isOnCooldown(this)) {
                    player.displayClientMessage(
                            Component.translatable("message.wastelayer.cooldown")
                ...
                player.displayClientMessage(
                        Component.translatable("message.wastelayer.blade_locked")
                                .withStyle(ChatFormatting.DARK_RED), true);
                return InteractionResultHolder.fail(stack);
            } else {
                player.displayClientMessage(
                        Component.translatable("message.wastelayer.insufficient_mana")
    ...
    private boolean canUseManaBlade(ItemStack stack) {
        if (ItemNBTHelper.getBoolean(stack, BLADE_LOCKED_TAG, false)) return false;
        return getMana(stack) >= BLADE_MANA_COST;
    }

    private double calculateAttackRange(ItemStack stack) {
        float manaPercent = (float) getMana(stack) / MAX_MANA;
    ...
    private void fireManaBlade(Level level, Player player, ItemStack stack, double range) {
        ...
        Effect slash = new EffectSlash(serverLevel.dimension().location().hashCode())
                .setSlashProperties(player.getYRot(), player.getXRot(),
                        30.0F + rand.nextFloat() * 120.0F, 2.5F, 2.0F, 150.0F)
                .setPosition(x1, y1, z1)
                .setMotion((x2 - x1) / 8.0, (y2 - y1) / 8.0, (z2 - z1) / 8.0)
                .setLife(8).setAdditive(true).setColor(0.2F, 0.0F, 0.4F, 1.0F);

        NetworkHandler.CHANNEL.send(PacketDistributor.ALL.noArg(),
                new EffectMessage(FXHandler.FX_SLASH, slash.write()));

        level.playSound(null, player.getX(), player.getY(), player.getZ(),
                ModSounds.WASTELAYER.get(), SoundSource.PLAYERS, 3.0F, 1.0F);
    ...
    private void spawnBladeParticles(ServerLevel level, Vec3 start, Vec3 direction, double range) {
        ...
        for (int i = 0; i < particleCount; i++) {
            ...
            WispParticleData data = WispParticleData.wisp(
                    0.6F + level.random.nextFloat() * 0.4F, 0.2f, 0.0f, 0.4f);
            level.sendParticles(data, pos.x, pos.y, pos.z, 3, 0.2, 0.2, 0.2, 0.08);
        }
    }
    ...
    private void performBladeDamage(ServerLevel level, Player player, double x, double y, double z,
        ...
        for (LivingEntity entity : level.getEntitiesOfClass(LivingEntity.class, damageBox)) {
            ...
            entity.hurt(ModDamageSources.trueDamage(level, player), BLADE_TRUE_DAMAGE);

            entity.addEffect(new MobEffectInstance(
                    ModEffects.ROOTED.get(), BLADE_ROOT_DURATION, 0, false, true, true));

            if (entity.getHealth() > 0.0F) {
                Random rand = new Random();
                Effect cut = new EffectCut(level.dimension().location().hashCode())
                        .setSlashProperties(player.getYRot(), player.getXRot(), rand.nextFloat() * 360.0F)
                        .setColor(0.2F, 0.0F, 0.4F, 1.0F)
                        .setPosition(entity.getX(), entity.getY() + entity.getBbHeight() / 2.0F, entity.getZ())
                        .setAdditive(true).setLife(15);
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (distance <= attackRange && distance > 3.0) {
            ...
            target.hurt(target.damageSources().playerAttack((Player) attacker), bonusDamage);
        }
    ...
    private void handlePlayerAttack(ItemStack stack, Player target, LivingEntity attacker) {
        ...
        if (sharpnessLevel > 0) {
            ...
            if (stolenMana > 0) {
                int newMana = Math.min(MAX_MANA, getMana(stack) + stolenMana);
                if (newMana >= MAX_MANA) {
                    ItemNBTHelper.setBoolean(stack, BLADE_LOCKED_TAG, false);
                    notifyBladeUnlocked(attacker);
                }
    ...
    private void notifyBladeUnlocked(LivingEntity entity) {
        if (entity instanceof Player player) {
            player.displayClientMessage(
                    Component.translatable("message.wastelayer.blade_unlocked")
    ...
    public void onEntityKilled(ItemStack stack, LivingEntity killed, LivingEntity killer) {
        ...
        int newMana = getMana(stack) + manaGain;
...
//                SoundEvents.EXPERIENCE_ORB_PICKUP, SoundSource.PLAYERS, 0.5F, 1.0F);
    ...
    private int calculateManaGain(LivingEntity killed, LivingEntity killer) {
        ...
        return Math.max((int) (BASE_MANA_PER_KILL + (maxHealth * MANA_PER_HP * difficulty)), BASE_MANA_PER_KILL);
    }
    ...
    private int stealManaFromPlayer(Player target, float percent) {
        ...
        for (ItemStack armor : target.getArmorSlots()) totalStolen += stealFromItem(armor, percent);
        for (int i = 0; i < target.getInventory().getContainerSize(); i++)
            totalStolen += stealFromItem(target.getInventory().getItem(i), percent);
        ...
        curiosOptional.ifPresent(handler -> {
            for (ICurioStacksHandler stacksHandler : handler.getCurios().values()) {
                for (int i = 0; i < stacksHandler.getSlots(); i++)
                    stolen[0] += stealFromItem(stacksHandler.getStacks().getStackInSlot(i), percent);
            }
        });
        return stolen[0];
    }
    ...
    private int stealFixedManaFromPlayer(Player target, int amount) {
        ...
        totalStolen += stolen; remaining -= stolen;
        ...
        for (int i = 0; i < target.getInventory().getContainerSize(); i++) {
            if (remaining <= 0) break;
            stolen = stealFixedFromItem(target.getInventory().getItem(i), remaining);
            totalStolen += stolen; remaining -= stolen;
        }
        if (remaining <= 0) return totalStolen;
        ...
        curiosOptional.ifPresent(handler -> {
            for (ICurioStacksHandler stacksHandler : handler.getCurios().values()) {
                ...
                for (int i = 0; i < stacksHandler.getSlots(); i++) {
                    if (finalRemaining[0] <= 0) return;
                    int s = stealFixedFromItem(stacksHandler.getStacks().getStackInSlot(i), finalRemaining[0]);
                    finalStolen[0] += s; finalRemaining[0] -= s;
                }
            }
        });
    ...
    private int stealFromItem(ItemStack stack, float percent) {
        ...
        stack.getCapability(BotaniaForgeCapabilities.MANA_ITEM).ifPresent(manaItem -> {
            int toSteal = (int) (manaItem.getMana() * percent);
            if (toSteal > 0) { manaItem.addMana(-toSteal); stolen[0] = toSteal; }
        });
        return stolen[0];
    }
    ...
    private int stealFixedFromItem(ItemStack stack, int amount) {
        ...
        final int[] stolen = {0};
        stack.getCapability(BotaniaForgeCapabilities.MANA_ITEM).ifPresent(manaItem -> {
            int toSteal = Math.min(manaItem.getMana(), amount);
            if (toSteal > 0) { manaItem.addMana(-toSteal); stolen[0] = toSteal; }
        });
        return stolen[0];
    }
    ...
    private int distributeManaToPlayer(Player player, int mana) {
        ...
        if (mainHand.getItem() != this) remaining -= distributeManaToItem(mainHand, remaining);
        ...
        curiosOptional.ifPresent(handler -> {
            for (ICurioStacksHandler stacksHandler : handler.getCurios().values()) {
                ...
                for (int i = 0; i < stacksHandler.getSlots(); i++) {
                    ...
                    if (!stack.isEmpty()) finalRemaining[0] -= distributeManaToItem(stack, finalRemaining[0]);
                }
            }
        });
    ...
    private int distributeManaToItem(ItemStack stack, int mana) {
        ...
        stack.getCapability(BotaniaForgeCapabilities.MANA_ITEM).ifPresent(manaItem -> {
            int canAdd = manaItem.getMaxMana() - manaItem.getMana();
            if (canAdd > 0) {
    ...
    private void createManaOverflowExplosion(LivingEntity entity) {
        ...
        if (entity instanceof Player player) {
            ...
            if (heldWeapon.getItem() == this) savedMana = getMana(heldWeapon);
        }
        ...
        for (LivingEntity target : level.getEntitiesOfClass(LivingEntity.class, area)) {
            ...
            if (target.distanceToSqr(pos) > OVERFLOW_RADIUS * OVERFLOW_RADIUS) continue;

            if (target instanceof Player player) removeTotemEffects(player);

            target.hurt(ModDamageSources.absolute(level), OVERFLOW_DAMAGE);
        ...
        if (heldWeapon != null && savedMana > 0) setMana(heldWeapon, savedMana);
    }

    private void removeTotemEffects(Player player) {
        ItemStack mainHand = player.getMainHandItem();
        ItemStack offHand = player.getOffhandItem();
        if (mainHand.getItem().toString().contains("totem")) mainHand.shrink(1);
        if (offHand.getItem().toString().contains("totem")) offHand.shrink(1);
        for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
            ItemStack stack = player.getInventory().getItem(i);
            if (stack.getItem().toString().contains("totem")) stack.shrink(1);
        }
    }
    ...
    private void spawnMultiStageParticleAnimation(ServerLevel level, Vec3 pos) {
        ...
        for (int tick = 0; tick < totalDuration; tick++) {
            ...
            level.getServer().tell(new net.minecraft.server.TickTask(tick, () -> {
                ...
                if (progress < 0.3) spawnConvergingFrame(level, pos, progress / 0.3);
                else if (progress < 0.8) spawnSpiralRisingFrame(level, pos, (progress - 0.3) / 0.5);
                else spawnTopBurstFrame(level, pos, (progress - 0.8) / 0.2);
            }));
        }
    }
    ...
    private void spawnConvergingFrame(ServerLevel level, Vec3 pos, double progress) {
        ...
        for (int i = 0; i < particleCount; i++) {
            ...
            WispParticleData data = WispParticleData.wisp(0.4F + (float) progress * 0.3F, r, g, b);
            level.sendParticles(data,
                    pos.x + x, pos.y + y + 1.0, pos.z + z,
                    1, -x * 0.08, -y * 0.08, -z * 0.08, 0.05);
        }
    }
    ...
    private void spawnSpiralRisingFrame(ServerLevel level, Vec3 pos, double progress) {
        ...
        for (int spiral = 0; spiral < spiralCount; spiral++) {
            ...
            for (int i = 0; i < particlesPerSpiral; i++) {
                ...
                double radius;
                if (heightRatio < 0.2) radius = heightRatio / 0.2 * OVERFLOW_RADIUS * 0.4;
                else if (heightRatio < 0.6) radius = OVERFLOW_RADIUS * (0.4 + (heightRatio - 0.2) / 0.4 * 0.5);
                else radius = OVERFLOW_RADIUS * (0.9 - (heightRatio - 0.6) / 0.4 * 0.7);
                ...
                WispParticleData data = WispParticleData.wisp(
                        0.6F - colorProgress * 0.3F,
                        0.6f + colorProgress * 0.3f,
                        0.1f + colorProgress * 0.4f,
                        0.8f - colorProgress * 0.3f);

                level.sendParticles(data,
                        pos.x + x, pos.y + particleHeight + 1.0, pos.z + z,
                        1, velocityX, 0.15, velocityZ, 0.08);
            }
        }
    ...
    private void spawnCenterPillarParticles(ServerLevel level, Vec3 pos, double progress, double maxHeight) {
        ...
        for (int i = 0; i < pillarParticles; i++) {
            WispParticleData data = WispParticleData.wisp(0.3F, 0.9f, 0.7f, 1.0f);
            level.sendParticles(data,
                    pos.x + (level.random.nextDouble() - 0.5) * 0.3,
                    pos.y + level.random.nextDouble() * currentHeight + 1.0,
    ...
    private void spawnTopBurstFrame(ServerLevel level, Vec3 pos, double progress) {
        ...
        for (int i = 0; i < burstParticles; i++) {
            ...
            WispParticleData data = WispParticleData.wisp(
                    0.5F - (float) progress * 0.3F, 1.0f, 0.5f + (float) progress * 0.3f, 0.8f);

            level.sendParticles(data,
                    pos.x + x * 0.3, pos.y + maxHeight + 1.0, pos.z + z * 0.3,
                    1, x * 0.1, y * 0.1, z * 0.1, 0.15);
        }

        if (progress > 0.5) spawnParticleRain(level, pos, maxHeight, progress);
    }
    ...
    private void spawnParticleRain(ServerLevel level, Vec3 pos, double height, double progress) {
        ...
        for (int i = 0; i < rainParticles; i++) {
            ...
            double radius = level.random.nextDouble() * OVERFLOW_RADIUS;
            WispParticleData data = WispParticleData.wisp(0.3F, 0.7f, 0.3f, 0.9f);
            level.sendParticles(data,
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("tooltip.wastelayer.mana",
                format.format(mana), format.format(MAX_MANA)).withStyle(ChatFormatting.AQUA));

        ChatFormatting percentFormatting = manaPercent >= 1.0f ? ChatFormatting.LIGHT_PURPLE :
                manaPercent >= 0.4f ? ChatFormatting.AQUA : ChatFormatting.RED;
        tooltip.add(Component.literal(String.format("%.1f%%", manaPercent * 100)).withStyle(percentFormatting));

        tooltip.add(Component.literal(""));
        tooltip.add(Component.translatable("tooltip.wastelayer.attack_range",
                String.format("%.1f", calculateAttackRange(stack))).withStyle(ChatFormatting.GOLD));

        tooltip.add(Component.literal(""));
        tooltip.add(Component.translatable("tooltip.wastelayer.ability1").withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("tooltip.wastelayer.ability2").withStyle(ChatFormatting.GRAY));

        boolean bladeLocked = ItemNBTHelper.getBoolean(stack, BLADE_LOCKED_TAG, false);
        if (bladeLocked) {
            tooltip.add(Component.literal(""));
            tooltip.add(Component.translatable("tooltip.wastelayer.blade_locked")
                    .withStyle(ChatFormatting.DARK_RED, ChatFormatting.BOLD));
            tooltip.add(Component.translatable("tooltip.wastelayer.blade_unlock_hint")
                    .withStyle(ChatFormatting.RED));
        } else if (mana >= BLADE_MANA_COST) {
            tooltip.add(Component.literal(""));
            tooltip.add(Component.translatable("tooltip.wastelayer.blade_ready")
                    .withStyle(ChatFormatting.LIGHT_PURPLE, ChatFormatting.BOLD));
            tooltip.add(Component.translatable("tooltip.wastelayer.blade_cost",
                    format.format(BLADE_MANA_COST)).withStyle(ChatFormatting.DARK_PURPLE));
            tooltip.add(Component.translatable("tooltip.wastelayer.blade_damage",
                    String.format("%.0f", BLADE_TRUE_DAMAGE)).withStyle(ChatFormatting.DARK_RED));
        } else {
            tooltip.add(Component.literal(""));
            tooltip.add(Component.translatable("tooltip.wastelayer.blade_insufficient")
        ...
        if (sharpness > 0) {
            ...
            tooltip.add(Component.literal(""));
            tooltip.add(Component.translatable("tooltip.wastelayer.sharpness",
                    String.format("%.0f%%", percent)).withStyle(ChatFormatting.GOLD));
        }

        if (mana > MAX_MANA * 0.9) {
            tooltip.add(Component.literal(""));
            tooltip.add(Component.translatable("tooltip.wastelayer.overflow")
    ...
    public int getBarColor(ItemStack stack) {
        ...
        boolean bladeLocked = ItemNBTHelper.getBoolean(stack, BLADE_LOCKED_TAG, false);
        if (bladeLocked) return ((int)(100 * fraction)) << 16;
        if (fraction >= 1.0f) return 0x8B00FF;
    ...
    private class WastelayerManaItem implements ManaItem, ICapabilityProvider {
        private final ItemStack stack;
        private final LazyOptional<ManaItem> holder = LazyOptional.of(() -> this);

        public WastelayerManaItem(ItemStack stack) { this.stack = stack; }

        @Override public int getMana() { return ItemNBTHelper.getInt(stack, MANA_TAG, 0); }
        @Override public int getMaxMana() { return MAX_MANA; }

        @Override
        public void addMana(int mana) {
            int newMana = Math.max(0, Math.min(MAX_MANA, getMana() + mana));
            ItemNBTHelper.setInt(stack, MANA_TAG, newMana);
            if (newMana >= MAX_MANA) ItemNBTHelper.setBoolean(stack, BLADE_LOCKED_TAG, false);
        }

        @Override public boolean canReceiveManaFromPool(BlockEntity pool) { return true; }
        @Override public boolean canReceiveManaFromItem(ItemStack otherStack) { return true; }
        @Override public boolean canExportManaToPool(BlockEntity pool) { return false; }
        @Override public boolean canExportManaToItem(ItemStack otherStack) { return false; }
        @Override public boolean isNoExport() { return true; }

        @Nonnull
        @Override
        public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
            return cap == BotaniaForgeCapabilities.MANA_ITEM ? holder.cast() : LazyOptional.empty();
        }
    }
}