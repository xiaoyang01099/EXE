...
public class Paradox extends SwordItem {
    ...
    public Paradox() {
        super(Tiers.NETHERITE, 3, -2.4F,
                new Properties().stacksTo(1).rarity(Rarity.EPIC).fireResistant());
    }
    ...
    public boolean onLeftClickEntity(ItemStack stack, Player player, Entity entity) {
        if (!player.level().isClientSide() && entity instanceof LivingEntity target) {
            ...
            if (randomDamage > 0) {
                target.hurt(ModDamageSources.paradox(player.level(), player), (float) randomDamage);
            }
            if (playerDamage > 0) {
                player.hurt(ModDamageSources.paradoxReflection(player.level()), (float) playerDamage);
            }
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (attacker instanceof Player player && !attacker.level().isClientSide()) {
            if (Math.random() < 0.1) {
                ...
                target.hurt(ModDamageSources.paradox(player.level(), player), (float) paradoxDamage);
                player.hurt(ModDamageSources.paradoxReflection(player.level()), (float) (10.0 - paradoxDamage));
