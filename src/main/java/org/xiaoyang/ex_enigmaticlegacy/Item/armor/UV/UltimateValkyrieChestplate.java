...
public class UltimateValkyrieChestplate extends UltimateValkyrie {
    public UltimateValkyrieChestplate(Properties properties) {
        super(ArmorItem.Type.CHESTPLATE, properties.fireResistant().stacksTo(1));
    }
    ...
    public void onArmorTick(ItemStack stack, Level level, Player player) {
        if (!level.isClientSide) {
            player.addEffect(new MobEffectInstance(
                    MobEffects.DAMAGE_RESISTANCE, 2, 3, false, false));

            float healthPercentage = player.getHealth() / player.getMaxHealth();
            if (healthPercentage < 0.5F) {
                int resistanceLevel = (int) ((1.0F - healthPercentage) * 10);
                player.addEffect(new MobEffectInstance(
                        MobEffects.DAMAGE_RESISTANCE, 2,
                        Math.min(resistanceLevel, 5), false, false));
            }
        }
