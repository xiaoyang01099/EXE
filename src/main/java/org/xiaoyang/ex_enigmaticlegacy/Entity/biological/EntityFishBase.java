...
public abstract class EntityFishBase extends TamableAnimal {
    ...
    public ItemStack getPropagule() {
        ...
        nbt.putString("creature", Registries.ENTITY_TYPE.registry().toString());
        stack.setTag(nbt);
        return stack;
    }
    ...
    public boolean checkSpawnObstruction(LevelReader level) {
        return level.getBlockState(this.blockPosition()).getFluidState().is(FluidTags.WATER);
    }

    public boolean isInWater() {
        return level().getBlockState(blockPosition()).getFluidState().is(FluidTags.WATER)
                || level().getBlockState(blockPosition().above()).getFluidState().is(FluidTags.WATER);
    }
    ...
    public boolean isCollidingRim() {
        if (this.isInWater()) {
            ...
            if (hitResult != null && hitResult.getType() == HitResult.Type.BLOCK) {
                ...
                return level().getBlockState(hitPos).getFluidState().is(FluidTags.WATER);
            }
        }
        return false;
    }
