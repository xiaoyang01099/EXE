...
import org.xiaoyang.ex_enigmaticlegacy.Network.inputMessage.LunarBurstMessage;
import org.xiaoyang.ex_enigmaticlegacy.Network.inputMessage.LunarFlaresParticleMessage;
...
public class EntityLunarFlare extends ThrowableProjectile {
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (result.getEntity() != this.getOwner() && !this.level().isClientSide) {
            result.getEntity().hurt(new ModDamageSources.DamageSourceMagic(this.level(), this.getOwner()), 100.0F);
        }
    }
    ...
    private void handleTargetHit(BlockPos targetPos) {
        ...
        for (LivingEntity entity : affectedEntities) {
            if (!entity.isRemoved()) {
                entity.hurt(new ModDamageSources.DamageSourceMagic(this.level(), this.getOwner()), 75.0F);
        ...
        this.level().levelEvent(2001, targetPos, Block.getId(blockState));
