                ...
                if (!this.level.isClientSide) {
                    ...
                    for (LivingEntity living : livingEntities) {
                        if (living != thrower && !living.isDeadOrDying()) {
                            if (player != null) {
                                living.hurt(new ModDamageSources.DamageSourceMagic(level(), player), DAMAGE_DIRECT);
                            }
    ...
    public void invokeDamageEffects() {
        ...
        for (LivingEntity target : targets) {
            if (!target.isDeadOrDying() && !this.level.isClientSide) {
                target.hurt(new ModDamageSources.DamageSourceMagic(level(), this.getOwner()), DAMAGE_IMPACT);
