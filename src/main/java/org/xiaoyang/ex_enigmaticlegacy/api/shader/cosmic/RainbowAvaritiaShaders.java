...
public final class RainbowAvaritiaShaders {
    ...
    public static final TextureAtlasSprite[] COSMIC_SPRITES = new TextureAtlasSprite[11];
    ...
    public static final ResourceLocation[] COSMIC_LOCATIONS = new ResourceLocation[11];
    public static boolean cosmicUVsInitialized = false;
    private static boolean particleCosmicUVsInitialized = false;


    public static void updateCosmicUVs(float[] target) {
        if (!cosmicUVsInitialized || target == null) {
            return;
        }

        System.arraycopy(COSMIC_UVS, 0, target, 0, Math.min(COSMIC_UVS.length, target.length));
    }
    ...
    private static void initCosmicUVs(TextureAtlas atlas) {
        for (int i = 0; i < COSMIC_LOCATIONS.length; i++) {
        ...
        try {
            event.registerShader(
                    CCShaderInstance.create(
                            ...
                            new ResourceLocation(Exe.MODID, "rainbow_cosmic"),
                            DefaultVertexFormat.BLOCK
                    ),
                    e -> {
            ...
            event.registerShader(
                    CCShaderInstance.create(
                            event.getResourceProvider(),
                            new ResourceLocation(Exe.MODID, "rainbow_cosmic_particle"),
