...
import org.xiaoyang.ex_enigmaticlegacy.api.test.curse.CorruptionEvent;
import org.xiaoyang.ex_enigmaticlegacy.api.test.curse.api.ICursedManaPool;
import org.xiaoyang.ex_enigmaticlegacy.api.test.curse.api.IManaConverter;
import org.xiaoyang.ex_enigmaticlegacy.api.test.curse.api.PoolCorruptionManager;
...
public class TileManaConverter extends BlockEntity implements IManaConverter {
    ...
    private void tickServer() {
        ...
        ManaPool normalPool = findNearbyNormalPool();
        ICursedManaPool cursedPool = findNearbyCursedPool();

        if (normalPool != null && cursedPool != null && normalPool != cursedPool) {
    ...
    private void convertNormalToCursedTick(ManaPool normalPool, ICursedManaPool cursedPool) {
        if (normalPool.getCurrentMana() >= CONVERSION_AMOUNT && !cursedPool.isCursedManaFull()) {
            converting = true;
            conversionProgress++;

            if (conversionProgress >= CONVERSION_TIME) {
                int available = Math.min(CONVERSION_AMOUNT,
                        cursedPool.getMaxCursedMana() - cursedPool.getCurrentCursedMana());
                if (available <= 0) {
                    conversionProgress = 0;
                    return;
                }
                int converted = convertNormalToCursed(available);
                normalPool.receiveMana(-available);
    ...
    private void convertCursedToNormalTick(ManaPool normalPool, ICursedManaPool cursedPool) {
        if (cursedPool.getCurrentCursedMana() >= CONVERSION_AMOUNT && !normalPool.isFull()) {
            ...
            if (conversionProgress >= CONVERSION_TIME) {
                int available = Math.min(CONVERSION_AMOUNT,
                        normalPool.getMaxMana() - normalPool.getCurrentMana());
                if (available <= 0) {
                    conversionProgress = 0;
                    return;
                }
                int converted = convertCursedToNormal(available);
                cursedPool.receiveCursedMana(-available);
    ...
    public void addCorruption(int amount) {
        ...
        if (level != null && !level.isClientSide) {
            ...
            if (oldCorruption < ManaCorruptionManager.MAX_CORRUPTION
                    && corruptionLevel >= ManaCorruptionManager.MAX_CORRUPTION
                    && oldCorruption >= 75) {
                MinecraftForge.EVENT_BUS.post(new CorruptionEvent.Critical(level, worldPosition, corruptionLevel));
            }
        }
    ...
    @Nullable
    private ManaPool findNearbyNormalPool() {
        ...
        for (int x = -3; x <= 3; x++) {
            for (int y = -3; y <= 3; y++) {
                for (int z = -3; z <= 3; z++) {
                    ...
                    if (tile instanceof ManaPool pool && !(tile instanceof ICursedManaPool)) {
                        return pool;
                    }
                }
            }
            ...
            for (int y = -5; y <= 5; y++) {
                for (int z = -5; z <= 5; z++) {
                    ...
                    if (tile instanceof TileManaConverter converter) {
                        ...
                        PoolCorruptionManager.addCorruption(level, checkPos, Math.max(1, spreadAmount));
                        reduceCorruption(1);
                    }
                }
            }
