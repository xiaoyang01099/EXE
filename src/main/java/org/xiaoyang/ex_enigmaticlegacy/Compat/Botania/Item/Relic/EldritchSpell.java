...
import org.xiaoyang.ex_enigmaticlegacy.Entity.others.EntityDarkMatterOrb;
...
public class EldritchSpell extends Item implements ICursed {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (Screen.hasShiftDown()) {
            tooltip.add(Component.translatable("item.ItemEldritchSpell1.lore").withStyle(ChatFormatting.DARK_PURPLE));
            tooltip.add(Component.translatable("item.ItemEldritchSpell2.lore").withStyle(ChatFormatting.DARK_PURPLE));
            tooltip.add(Component.translatable("item.ItemEldritchSpell3.lore").withStyle(ChatFormatting.DARK_PURPLE));
            tooltip.add(Component.translatable(""));
            tooltip.add(Component.translatable("item.ItemEldritchSpell4.lore").withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.translatable("item.ItemEldritchSpell5.lore",
                    ELDRITCH_SPELL_DAMAGE).withStyle(ChatFormatting.RED));
            tooltip.add(Component.translatable("item.ItemEldritchSpell6.lore").withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.translatable(""));
        } else if (Screen.hasControlDown()) {
            tooltip.add(Component.translatable("consumed_per_cast:").withStyle(ChatFormatting.AQUA));
            tooltip.add(Component.translatable(" • " + EXPERIENCE_COST + " XP").withStyle(ChatFormatting.GREEN));
        } else {
            tooltip.add(Component.translatable("item.FRShiftTooltip.lore").withStyle(ChatFormatting.YELLOW));
            tooltip.add(Component.translatable("item.FRCtrlTooltip.lore").withStyle(ChatFormatting.AQUA));
        }
    }
