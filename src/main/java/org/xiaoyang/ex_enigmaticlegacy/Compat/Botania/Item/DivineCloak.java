...
import org.xiaoyang.ex_enigmaticlegacy.Network.inputMessage.BlinkMessage;
...
public class DivineCloak extends BaubleItem implements ICurioItem, ICurioRenderer {
    ...
    public static final int HEIMDALL = 3;
    public static final int LOKI = 4;
    private static final String TAG_TYPE = "cloak_type";
    private final int cloakType;

    public DivineCloak(Properties properties, int cloakType) {
    ...
    public static final ResourceKey<DamageType> EARTHQUAKE_TYPE = ResourceKey.create(
            Registries.DAMAGE_TYPE,
            new ResourceLocation(Exe.MODID, "earthquake")
    );
    ...
    private static final ResourceLocation[] CLOAK_TEXTURES = {
            new ResourceLocation(Exe.MODID, "textures/models/cloak/njord_cloak.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/idunn_cloak.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/thor_cloak.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/heimdall_cloak.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/loki_cloak.png")
    };

    private static final ResourceLocation[] CLOAK_GLOW_TEXTURES = {
            new ResourceLocation(Exe.MODID, "textures/models/cloak/njord_cloak_glow.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/idunn_cloak_glow.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/thor_cloak_glow.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/heimdall_cloak_glow.png"),
            new ResourceLocation(Exe.MODID, "textures/models/cloak/loki_cloak_glow.png")
    };
    ...
    public ResourceLocation getCloakTexture(ItemStack stack) {
        ...
        return type >= 0 && type < CLOAK_TEXTURES.length ? CLOAK_TEXTURES[type] : CLOAK_TEXTURES[0];
    }

    public ResourceLocation getCloakGlowTexture(ItemStack stack) {
        int type = getCloakType(stack);
        return type >= 0 && type < CLOAK_GLOW_TEXTURES.length ? CLOAK_GLOW_TEXTURES[type] : CLOAK_GLOW_TEXTURES[0];
    }
    ...
    public static void onFall(LivingFallEvent event) {
        ...
        int type = ((DivineCloak) cloak.getItem()).getCloakType(cloak);
        ...
        if (type == NJORD) {
            ...
            if (damage > 0.0f) {
                ...
                DamageSource earthquakeSource = new DamageSource(
                        player.level().registryAccess()
                                .registryOrThrow(Registries.DAMAGE_TYPE)
                                .getHolderOrThrow(EARTHQUAKE_TYPE)
                );

                AABB aabb = player.getBoundingBox().inflate(4.0, 3.0, 4.0);
                List<LivingEntity> entities = player.level().getEntitiesOfClass(
                        LivingEntity.class, aabb,
                        e -> e != player && e.onGround()
                );
                for (LivingEntity e : entities) {
                    e.hurt(earthquakeSource, damage * 2);
                }
                player.hurt(earthquakeSource, 0.00005f);
    ...
    public static void onDamage(LivingAttackEvent event) {
        ...
        if (type == IDUNN && event.getSource().getEntity() != null) {
            ...
            if (src.is(DamageTypes.EXPLOSION) || src.is(DamageTypes.PLAYER_EXPLOSION)
                    ...
                    || src.is(DamageTypes.LAVA) || src.is(DamageTypes.FIREBALL)) {
                event.setCanceled(true);
            }
        }
    }
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(
        ...
        if (player.isCrouching()) {
            ...
            matrixStack.mulPose(Axis.XP.rotationDegrees(28.0f));
        }
