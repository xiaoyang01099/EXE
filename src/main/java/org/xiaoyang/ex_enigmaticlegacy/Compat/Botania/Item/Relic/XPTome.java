...
public class XPTome extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltipComponents, TooltipFlag isAdvanced) {
        ...
        if (!ItemNBTHelper.getBoolean(stack, TAG_ACTIVE, false)) {
            modeText = Component.translatable("item.ItemXPTomeDeactivated.lore").withStyle(ChatFormatting.RED);
        } else if (ItemNBTHelper.getBoolean(stack, TAG_ABSORPTION, true)) {
            modeText = Component.translatable("item.ItemXPTomeAbsorption.lore").withStyle(ChatFormatting.GREEN);
        } else {
            modeText = Component.translatable("item.ItemXPTomeExtraction.lore").withStyle(ChatFormatting.BLUE);
        }

        if (net.minecraft.client.gui.screens.Screen.hasShiftDown()) {
            tooltipComponents.add(Component.translatable("item.ItemXPTome1.lore").withStyle(ChatFormatting.GRAY));
            tooltipComponents.add(Component.nullToEmpty(""));
            tooltipComponents.add(Component.translatable("item.ItemXPTome2.lore").withStyle(ChatFormatting.GRAY));
            tooltipComponents.add(Component.translatable("item.ItemXPTome3.lore").withStyle(ChatFormatting.GRAY));
            tooltipComponents.add(Component.translatable("item.ItemXPTome4.lore").withStyle(ChatFormatting.GRAY));
            tooltipComponents.add(Component.nullToEmpty(""));
            tooltipComponents.add(Component.translatable("item.ItemXPTome5.lore").withStyle(ChatFormatting.GRAY));
            tooltipComponents.add(Component.translatable("item.ItemXPTome6.lore").withStyle(ChatFormatting.GRAY));
            tooltipComponents.add(Component.nullToEmpty(""));
            tooltipComponents.add(Component.translatable("item.ItemXPTome7.lore").withStyle(ChatFormatting.YELLOW));
            tooltipComponents.add(Component.translatable("item.ItemXPTome8.lore").withStyle(ChatFormatting.YELLOW));
            tooltipComponents.add(Component.translatable("item.ItemXPTome9.lore").withStyle(ChatFormatting.YELLOW));
        } else {
            tooltipComponents.add(Component.translatable("item.FRShiftTooltip.lore").withStyle(ChatFormatting.GRAY));
        }

        tooltipComponents.add(Component.nullToEmpty(""));
        tooltipComponents.add(Component.translatable("item.ItemXPTomeMode.lore")
                .append(" ").append(modeText));
        tooltipComponents.add(Component.nullToEmpty(""));
        tooltipComponents.add(Component.translatable("item.ItemXPTomeExp.lore").withStyle(ChatFormatting.AQUA));

        int storedXP = ItemNBTHelper.getInt(stack, TAG_XP_STORED, 0);
        int level = ExperienceHelper.getLevelForExperience(storedXP);
        tooltipComponents.add(Component.translatable("item.FRCode6.lore")
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slotId, boolean isSelected) {
        if (!level.isClientSide && entity instanceof Player player) {
            ...
            if (relicCap.isPresent()) {
                Relic relicInstance = relicCap.orElse(null);
