...
public class EntityAdvancedSpark extends SparkBaseEntity implements ManaSpark {
    ...
    public void tick() {
        ...
        List<ManaSpark> allSparks = null;

        if (isFirst || upgrade == SparkUpgradeType.RECESSIVE || upgrade == SparkUpgradeType.DOMINANT) {
            DyeColor network = this.getNetwork();
            allSparks = new ArrayList<>(SparkHelper.getSparksAround(level, getX(), getY(), getZ(), network));
        }
        ...
        if (!transfers.isEmpty() && !level.isClientSide) {
            ManaReceiver attachedReceiver = getAttachedManaReceiver();
            if (attachedReceiver != null) {
                ...
                if (manaForEach > 0) {
                    for (ManaSpark spark : transfers) {
                        SparkAttachable sparkAttachable = spark.getAttachedTile();
                        if (sparkAttachable != null && !sparkAttachable.areIncomingTranfersDone()) {
                            ...
                            if (availableSpace > 0) {
                                ...
                                if (sparkAttachable instanceof ManaReceiver) {
                                    ((ManaReceiver) sparkAttachable).receiveMana(spend);
                                }
                                manaSpent += spend;
    ...
    private void handleDispersiveUpgrade() {
        ...
        if (!receivingPlayers.isEmpty()) {
            ...
            if (attachedReceiver != null) {
                ...
                ((ManaItem) stack.getItem()).addMana(manaToPut);
    ...
    private void handleDominantUpgrade(List<ManaSpark> allSparks) {
        if (allSparks == null) return;

        List<ManaSpark> validSparks = allSparks.stream()
                .filter(spark -> spark != this)
                .filter(spark -> spark.getUpgrade() == SparkUpgradeType.NONE)
                .filter(spark -> spark.getAttachedTile() instanceof ManaPool)
                .toList();

        if (!validSparks.isEmpty()) {
            ManaSpark targetSpark = validSparks.get(level.getRandom().nextInt(validSparks.size()));
            targetSpark.registerTransfer(this);
        }
    }

    private void handleRecessiveUpgrade(List<ManaSpark> allSparks) {
        if (allSparks == null) return;

        for (ManaSpark spark : allSparks) {
            if (spark != this) {
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        ...
        if (!stack.isEmpty()) {
            ...
            if (stack.getItem() == BotaniaItems.twigWand) {
                if (player.isShiftKeyDown()) {
                    if (currentUpgrade != SparkUpgradeType.NONE) {
                        if (!level.isClientSide) {
                            spawnAtLocation(new ItemStack(BotaniaItems.sparkUpgradeDispersive, 1), 0.0F);
                        }
                ...
                for (DyeColor color : DyeColor.values()) {
                    for (ManaSpark spark : SparkHelper.getSparksAround(level, getX(), getY(), getZ(), color)) {
                        if (spark instanceof Entity sparkEntity) {
                            createParticleBeam(this, sparkEntity);
                        }
                    }
                }
                return InteractionResult.SUCCESS;
            ...
            if (stack.getItem() == BotaniaItems.sparkUpgradeDispersive && currentUpgrade == SparkUpgradeType.NONE) {
    ...
    private boolean handlePhantomInk(ItemStack stack) {
        if (!stack.isEmpty() && stack.getItem() == BotaniaItems.phantomInk && !level.isClientSide) {
    ...
    public void remove(RemovalReason reason) {
        ...
        if (!level.isClientSide && reason != RemovalReason.CHANGED_DIMENSION) {
            spawnAtLocation(new ItemStack(ModItems.ADVANCED_SPARK.get()), 0.0F);
            SparkUpgradeType upgrade = getUpgrade();
            if (upgrade != SparkUpgradeType.NONE) {
                spawnAtLocation(new ItemStack(BotaniaItems.sparkUpgradeDispersive, 1), 0.0F);
            }
    ...
    public @Nullable SparkAttachable getAttachedTile() {
        ...
        if (level.getBlockEntity(pos) instanceof SparkAttachable attachable) {
            return attachable;
        }
        return null;
    }
    ...
    public Collection<ManaSpark> getOutgoingTransfers() {
        Set<ManaSpark> toRemove = new HashSet<>();
    ...
    private boolean isValidTransferTarget(SparkUpgradeType thisUpgrade, SparkUpgradeType targetUpgrade, SparkAttachable targetAttachable) {
        ...
        if (thisUpgrade == SparkUpgradeType.RECESSIVE &&
            ...
            return !(targetAttachable instanceof ManaPool);
        }
        return false;
    }
    ...
    public boolean areIncomingTransfersDone() {
        SparkAttachable attachable = getAttachedTile();
        if (attachable instanceof ManaPool) {
    ...
    public @Nullable ManaReceiver getAttachedManaReceiver() {
        BlockPos pos = getAttachPos();
        if (level.getBlockEntity(pos) instanceof ManaReceiver receiver) {
            return receiver;
        }
        return null;
    }
