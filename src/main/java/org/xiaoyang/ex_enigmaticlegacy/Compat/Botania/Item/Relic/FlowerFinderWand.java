...
import org.xiaoyang.ex_enigmaticlegacy.Entity.others.NatureBoltEntity;
...
public class FlowerFinderWand extends Item implements INoEMCItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.tooltip.1"));
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.tooltip.2"));
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.tooltip.3"));
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.tooltip.4"));
    }
    ...
    private boolean shootNatureBolt(Level level, Player player, ItemStack stack) {
        ...
        if (!hasMana(player, RANGED_ATTACK_MANA_COST)) {
            if (!level.isClientSide) {
                player.displayClientMessage(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.no_mana"), true);
            }
            return false;
        }
    ...
    private boolean handleSelfHeal(Level level, Player player, ItemStack stack) {
        ...
        if (player.getHealth() >= player.getMaxHealth()) {
            if (!level.isClientSide) {
                player.displayClientMessage(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.full_health"), true);
            }
            return false;
        }
    ...
    private boolean handleAOEHeal(Level level, Player player, ItemStack stack) {
        ...
        if (!level.isClientSide) {
            ...
            if (!anyHealed) {
                player.displayClientMessage(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.no_targets"), true);
                return false;
            }
    ...
    public boolean handleAttack(Level level, Player player, Entity target, ItemStack stack) {
        ...
        if (!level.isClientSide && target instanceof LivingEntity) {
            ...
            // 造成伤害
            livingTarget.hurt(player.damageSources().indirectMagic(player, player), DAMAGE_AMOUNT);
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity entity, int timeLeft) {
        if (entity instanceof Player && entity.isCrouching()) {
            ...
            if (useDuration >= 40) { // 完全蓄力 (2秒)
                ...
                // 蓄力不足，返回
                if (!level.isClientSide) {
                    player.displayClientMessage(Component.translatable("item.ex_enigmaticlegacy.flower_finder_wand.charge_more"), true);
                }
                return;
            }
