...
import org.xiaoyang.ex_enigmaticlegacy.Entity.others.EntityCrimsonOrb;
...
public class CrimsonSpell extends Item implements ICursed {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (Screen.hasShiftDown()) {
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.lore1")
                    .withStyle(ChatFormatting.DARK_RED));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.lore2")
                    .withStyle(ChatFormatting.DARK_RED));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.lore3")
                    .withStyle(ChatFormatting.DARK_RED));
            tooltip.add(Component.nullToEmpty(""));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.lore4")
                    .withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.lore5")
                    .withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.damage_range",
                    (int)CRIMSON_SPELL_DAMAGE_MIN, (int)CRIMSON_SPELL_DAMAGE_MAX)
                    .withStyle(ChatFormatting.RED));
        } else if (Screen.hasControlDown()) {
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.cooldown", COOLDOWN_TICKS / 20)
                    .withStyle(ChatFormatting.AQUA));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.crimson_spell.range", SEARCH_DISTANCE)
                    .withStyle(ChatFormatting.AQUA));
        } else {
            tooltip.add(Component.translatable("item.FRCtrlTooltip.lore")
                    .withStyle(ChatFormatting.GRAY));
        ...
        if (!level.isClientSide && !RelicsEventHandler.isOnCoodown(player)) {
            ...
            if (player.experienceLevel >= expCost || player.isCreative()) {
                ...
                player.displayClientMessage(
                        Component.translatable("item.ex_enigmaticlegacy.crimson_spell.insufficient_exp", expCost)
