...
public class DeificAmulet extends Item implements ICurioItem, INoEMCItem {
        ...
        if (Screen.hasShiftDown()) {
            if (EFFECT_IMMUNITY_ENABLED) {
                if (ONLY_NEGATE_DEBUFFS) {
                    tooltip.add(Component.translatable("item.ItemDeificAmulet1_alt.lore")
                            .withStyle(ChatFormatting.GOLD));
                } else {
                    tooltip.add(Component.translatable("item.ItemDeificAmulet1.lore")
                            .withStyle(ChatFormatting.GOLD));
                }
            }
    ...
    public List<Component> getAttributesTooltip(List<Component> tooltips, ItemStack stack) {
        tooltips.add(Component.translatable("curios.tooltip.immunity")
                .withStyle(ChatFormatting.YELLOW));
        if (INVINCIBILITY_ENABLED) {
            tooltips.add(Component.translatable("curios.tooltip.invincibility")
                    .withStyle(ChatFormatting.YELLOW));
        }
        return tooltips;
    }
