...
public class FullAltarRender implements BlockEntityRenderer<FullAltarTile> {
    ...
    @Override
    public void render(@Nonnull FullAltarTile altar, float pticks, PoseStack ms,
        ...
        if (water) {
            ...
            TextureAtlasSprite sprite = this.blockRenderDispatcher
    ...
    private void renderIcon(PoseStack ms, VertexConsumer builder, TextureAtlasSprite sprite,
                            int color, float alpha, int overlay, int light) {
        ...
        builder.vertex(mat, 3, 13, 0).color(red, green, blue, a)
                .uv(sprite.getU(start), sprite.getV(end))
                .overlayCoords(overlay).uv2(light).normal(0, 0, 1).endVertex();
        builder.vertex(mat, 13, 13, 0).color(red, green, blue, a)
                .uv(sprite.getU(end), sprite.getV(end))
                .overlayCoords(overlay).uv2(light).normal(0, 0, 1).endVertex();
        builder.vertex(mat, 13, 3, 0).color(red, green, blue, a)
                .uv(sprite.getU(end), sprite.getV(start))
                .overlayCoords(overlay).uv2(light).normal(0, 0, 1).endVertex();
        builder.vertex(mat, 3, 3, 0).color(red, green, blue, a)
                .uv(sprite.getU(start), sprite.getV(start))
                .overlayCoords(overlay).uv2(light).normal(0, 0, 1).endVertex();
    }
}