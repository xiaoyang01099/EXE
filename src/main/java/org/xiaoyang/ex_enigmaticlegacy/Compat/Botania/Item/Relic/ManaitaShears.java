...
public class ManaitaShears extends ShearsItem {
        ...
        if (!level.isClientSide && player.isShiftKeyDown()) {
            ...
            if (currentRange >= MAX_RANGE) {
                ...
                player.displayClientMessage(
                        Component.translatable("item.ex_enigmaticlegacy.manaita_shears.message.mode_switch", 1, 1)
                                .withStyle(ChatFormatting.AQUA), false);
            } else {
                int newRange = currentRange + 2;
                setRange(stack, newRange);
                player.displayClientMessage(
                        Component.translatable("item.ex_enigmaticlegacy.manaita_shears.message.mode_switch", newRange, newRange)
                                .withStyle(ChatFormatting.AQUA), false);
            }

            level.playSound(null, player.getX(), player.getY(), player.getZ(),
                    SoundEvents.UI_BUTTON_CLICK.get(), SoundSource.PLAYERS, 0.5F, 1.0F);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.manaita_shears.tooltip.mode", range, range)
                .withStyle(ChatFormatting.AQUA));

        String doublingStatus = doubling ?
                Component.translatable("item.ex_enigmaticlegacy.manaita_shears.tooltip.doubling.on").getString() :
                Component.translatable("item.ex_enigmaticlegacy.manaita_shears.tooltip.doubling.off").getString();

        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.manaita_shears.tooltip.doubling", doublingStatus)
                .withStyle(doubling ? ChatFormatting.GREEN : ChatFormatting.RED));

        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.manaita_shears.tooltip.usage")
                .withStyle(ChatFormatting.GRAY));

        if(Screen.hasShiftDown()) {
            tooltip.add(Component.nullToEmpty(""));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.manaita_shears.desc")
                    .withStyle(ChatFormatting.YELLOW));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.manaita_shears.lore1")
                    .withStyle(ChatFormatting.DARK_PURPLE, ChatFormatting.ITALIC));
            tooltip.add(Component.translatable("item.ex_enigmaticlegacy.manaita_shears.lore2")
                    .withStyle(ChatFormatting.DARK_PURPLE, ChatFormatting.ITALIC));
        } else {
