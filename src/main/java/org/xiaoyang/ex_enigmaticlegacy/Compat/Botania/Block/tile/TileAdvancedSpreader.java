    ...
    private static void serverTick(Level level, BlockPos pos, BlockState state, TileAdvancedSpreader spreader) {
        if (!spreader.canShoot) {
            if (spreader.pingbackTicks <= 0) {
                ...
                var bursts = level.getEntitiesOfClass(ManaBurstEntity.class, aabb);
                ManaBurstEntity found = null;
                for (ManaBurstEntity burst : bursts) {
                    ManaBurst manaBurst = burst;
    ...
    public static class WandHud implements WandHUD {
        ...
        public void renderHUD(GuiGraphics guiGraphics, Minecraft mc) {
            ...
            if (!lens.isEmpty()) {
                ...
                guiGraphics.drawString(mc.font, lensName, x + 20, y + 5, color);
            ...
            if (spreader.receiver != null) {
                ...
                if (!receiverStack.isEmpty()) {
                    ...
                    guiGraphics.drawString(mc.font, stackName, x + 20, y + 5, color);

                    guiGraphics.renderItem(receiverStack, x, y);
                }
            }
