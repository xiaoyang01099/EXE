...
public class RenderTileAdvancedSpreader implements BlockEntityRenderer<TileAdvancedSpreader> {
    ...
    @Override
    public void render(@Nonnull TileAdvancedSpreader spreader, float partialTicks, PoseStack ms,
                       MultiBufferSource buffers, int light, int overlay) {
        ms.pushPose();
        ms.translate(0.5, 0.5, 0.5);

        Quaternionf transform = Axis.YP.rotationDegrees(spreader.rotationX + 90.0F);
        transform.mul(Axis.XP.rotationDegrees(spreader.rotationY));
        ...
        if (spreader.getAdvancedVariant() == BlockAdvancedSpreader.VariantN.NATURE) {
            int color = Mth.hsvToRgb(
                    (float)((time * 2.0 + (double)(new Random(spreader.getBlockPos().hashCode()))
                            .nextInt(10000)) % 360.0) / 360.0F,
                    0.4F, 0.9F
            );
        ...
        VertexConsumer buffer = buffers.getBuffer(
                ItemBlockRenderTypes.getRenderType(spreader.getBlockState(), false)
        );

        BakedModel spreaderModel = Minecraft.getInstance()
                .getBlockRenderer()
                .getBlockModel(spreader.getBlockState());
        ...
        ms.pushPose();
        ms.translate(0.5, 0.5, 0.5);
        ms.mulPose(Axis.YP.rotationDegrees((float)time % 360.0F));
        ms.translate(-0.5, -0.5, -0.5);
        ...
        if ((Boolean) spreader.getBlockState().getValue(BotaniaStateProperties.HAS_SCAFFOLDING)) {
    ...
    private BakedModel getCoreModel(TileAdvancedSpreader tile) {
        return switch (tile.getAdvancedVariant()) {
            case NATURE -> SpecialMiscellaneousModels.INSTANCE.advancedSpreaderCore;
        };
    }
