...
public class GoldenLaurel extends RelicItem implements ICurioItem, INoEMCItem {
        ...
        if (level.isClientSide) {
            ...
            if (level.getGameTime() % 3 == 0) {
                ...
                for (BlockPos pos : BlockPos.betweenClosed(playerPos.offset(-range, 0, -range), playerPos.offset(range, 1, range))) {
                    ...
                    if (blockEntity instanceof ManaPoolBlockEntity tilePool) {
                        if (!tilePool.isFull())
                            tilePool.receiveMana(1600);
                    } else if (blockEntity instanceof FunctionalFlowerBlockEntity functionalFlower) {
                        if (functionalFlower.getMana() < functionalFlower.getMaxMana()) {
                            functionalFlower.addMana(1600);
                        }
                    } else if (blockEntity instanceof GeneratingFlowerBlockEntity generatingFlower) {
                        if (generatingFlower.getMana() < generatingFlower.getMaxMana()) {
                            generatingFlower.addMana(1600);
                        }
                    } else if (blockEntity instanceof TerrestrialAgglomerationPlateBlockEntity terraPlate) {
    ...
    private void handleManaAndFlowers(Player player) {
        ...
        for (BlockPos pos : BlockPos.betweenClosed(
            ...
            if (world.getBlockEntity(pos) instanceof ManaReceiver manaReceiver) {

                if (manaReceiver instanceof FunctionalFlowerBlockEntity functionalFlower) {
                    functionalFlower.addMana(MANA_PER_TICK);
                    functionalFlower.tickFlower();
                }
                else if (manaReceiver instanceof GeneratingFlowerBlockEntity generatingFlower) {
                ...
                if (relicCap.isPresent()) {
                    ...
                    if (relic != null && relic.isRightPlayer(player)) {
                        ...
                        player.sendSystemMessage(
                                Component.translatable("message.ex_enigmaticlegacy.laurel_protection")
    ...
    private void fillManaInRange(Player player) {
        ...
        for (BlockPos pos : BlockPos.betweenClosed(
            ...
            if (world.getBlockEntity(pos) instanceof ManaReceiver manaReceiver) {
                ...
                } else if (manaReceiver instanceof ManaPool pool) {
                    for (int i = 0; i < 5; i++) {
            ...
            if (world.getBlockEntity(pos) instanceof GeneratingFlowerBlockEntity generatingFlower) {
    ...
    public void onEquipped(ItemStack stack, LivingEntity entity) {
        if (entity instanceof Player player) {
            player.sendSystemMessage(
                    Component.translatable("message.ex_enigmaticlegacy.laurel_blessing")
                            .withStyle(ChatFormatting.GOLD)
            );
        }
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world,
        ...
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.golden_laurel.desc1")
                .withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.golden_laurel.desc2")
                .withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("item.ex_enigmaticlegacy.golden_laurel.desc3")
                .withStyle(ChatFormatting.GRAY));
    }
}