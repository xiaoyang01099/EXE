...
public class TileLebethronCore extends BotaniaBlockEntity implements IRenderHud {
    ...
    private boolean checkBlock(BlockPos pos) {
        ...
        return state.is(ModBlocks.LEBETHRON_CORE.get());
    }
    ...
    @Override
    @OnlyIn(Dist.CLIENT)
    public void renderHud(Minecraft mc, GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
        if (!validTree) {
            return;
        }
        ...
        guiGraphics.fill(x - 2, y - 2, x + 18, y + 18, 0x44000000);
        guiGraphics.fill(x, y, x + 16, y + 16, 0x44000000);

        guiGraphics.renderItem(displayStack, x, y);

        if (getStoredBlock() != null) {
            guiGraphics.drawString(mc.font, "✓", x + 10, y + 9, 0x004C00, false);
            guiGraphics.drawString(mc.font, "✓", x + 10, y + 8, 0x00BD4D, false);
        } else {
            guiGraphics.drawString(mc.font, "✗", x + 10, y + 9, 0x4C0000, false);
            guiGraphics.drawString(mc.font, "✗", x + 10, y + 8, 0xD1948D, false);
        }
