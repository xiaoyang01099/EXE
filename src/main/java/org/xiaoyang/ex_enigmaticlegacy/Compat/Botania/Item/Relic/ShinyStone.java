...
import org.xiaoyang.ex_enigmaticlegacy.Entity.others.EntityShinyEnergy;
...
public class ShinyStone extends Item implements ICurioItem, INoEMCItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (Screen.hasShiftDown()) {
            tooltip.add(Component.translatable("item.ItemShinyStone1.lore").withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.translatable("item.ItemShinyStone2.lore").withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.nullToEmpty(""));

            boolean isPermanentDay = ItemNBTHelper.getBoolean(stack, "PermanentDay", false);
            if (isPermanentDay) {
                tooltip.add(Component.translatable("item.shinystone.mode.day").withStyle(ChatFormatting.GOLD));
            } else {
                tooltip.add(Component.translatable("item.shinystone.mode.normal").withStyle(ChatFormatting.AQUA));
            }

            tooltip.add(Component.translatable("item.shinystone.key_hint").withStyle(ChatFormatting.YELLOW));
        } else {
        ...
        if (level.getServer() != null) {
            if (newState) {
                ...
                player.displayClientMessage(
                        Component.translatable("item.daynight.controller1")
                ...
                player.displayClientMessage(
                        Component.translatable("item.daynight.controller2")
