...
public class SlimeNecklace extends Item implements ICurioItem, IWaveName {
    ...
    private void tickSlimeAffinity(CompoundTag tag, Player player, Level level) {
        ...
        for (Slime slime : slimes) {
            ...
            if (dist > SLIME_FOLLOW_DISTANCE + 1.0) {
                ...
                if (slime.onGround() && level.random.nextInt(5) == 0) {
    ...
    private void tickAutoAbsorb(CompoundTag tag, Player player, ItemStack cannon, Level level) {
        ...
        player.displayClientMessage(
                Component.translatable("message.slime_necklace.auto_absorbed", size)
    ...
    private void tickPassiveGen(CompoundTag tag, Player player, ItemStack cannon) {
        ...
        if (timer >= PASSIVE_GEN_INTERVAL) {
            ...
            player.displayClientMessage(
                    Component.translatable("message.slime_necklace.condensed")
        ...
        if (timer >= FUSION_INTERVAL) {
            ...
            for (int size = 1; size < SlimeCannon.MAX_SLIME_SIZE; size++) {
                ...
                if (count >= FUSION_COST_COUNT) {
                    ...
                    player.displayClientMessage(
                            Component.translatable("message.slime_necklace.fused", size, nextSize)
...
//            target.addEffect(new MobEffectInstance(ModEffects.ROOTED.get(), TIME_FREEZE_DURATION, 1, true, true, false));
        ...
        player.displayClientMessage(
                Component.translatable("message.slime_necklace.time_freeze", frozenCount)
    ...
    public static boolean tryReflectProjectile(Player player, DamageSource source, float amount) {
        ...
        if (directEntity != null) {
            ...
            if (sourceEntity instanceof LivingEntity living && sourceEntity != player) {
                living.hurt(player.damageSources().thorns(player), amount * REFLECT_DAMAGE_MULT);
            }
        }
        ...
        player.displayClientMessage(
                Component.translatable("message.slime_necklace.reflected",
    ...
    public static boolean tryActivateSlimeRain(Player player) {
        ...
        player.displayClientMessage(
                Component.translatable("message.slime_necklace.rain_activated")
    ...
    private void tickSlimeRain(CompoundTag tag, Player player, ItemStack cannon, Level level) {
        if (!tag.getBoolean(TAG_RAIN_ACTIVE)) {
            if (player.isShiftKeyDown() && player.getXRot() < -60.0F) {
                ...
                if (chargeTime >= 60) {
                    ...
                    player.displayClientMessage(
                            Component.translatable("message.slime_necklace.rain_charging",
                                            3 - chargeTime / 20)
        ...
        if (fired >= RAIN_PROJECTILE_COUNT) {
            ...
            player.displayClientMessage(
                    Component.translatable("message.slime_necklace.rain_complete")
                            .withStyle(ChatFormatting.GREEN),
                    true);
        }
    }
    ...
    private void spawnRainProjectile(Player player, Level level) {
        ...
        level.playSound(null, BlockPos.containing(ball.getX(), ball.getY(), ball.getZ()),
    ...
    public static boolean trySlimeShield(Player player, DamageSource source, float amount) {
        ...
        player.displayClientMessage(
                Component.translatable("message.slime_necklace.shield_triggered")
                        .withStyle(ChatFormatting.LIGHT_PURPLE),
                true);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level,
        ...
        tooltip.add(Component.translatable("tooltip.slime_necklace.title")
                .withStyle(ChatFormatting.GREEN));
        tooltip.add(Component.literal(""));

        tooltip.add(Component.translatable("tooltip.slime_necklace.affinity")
                .withStyle(ChatFormatting.GREEN));
        tooltip.add(Component.translatable("tooltip.slime_necklace.affinity_desc",
                        String.format("%.0f", SLIME_AFFINITY_RADIUS),
                        String.format("%.0f", SLIME_FOLLOW_DISTANCE))
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.auto_absorb",
                        String.format("%.0f", AUTO_ABSORB_RADIUS))
                .withStyle(ChatFormatting.AQUA));
        tooltip.add(Component.translatable("tooltip.slime_necklace.auto_absorb_desc")
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.absorb_enhance",
                        String.format("%.0f%%", (1 - ABSORB_MANA_DISCOUNT) * 100),
                        String.format("%.0f%%", (1 - ABSORB_COOLDOWN_MULTIPLIER) * 100))
                .withStyle(ChatFormatting.AQUA));

        tooltip.add(Component.translatable("tooltip.slime_necklace.passive_gen",
                        PASSIVE_GEN_INTERVAL / 20, PASSIVE_GEN_MANA_COST, PASSIVE_GEN_CAP)
                .withStyle(ChatFormatting.AQUA));
        tooltip.add(Component.translatable("tooltip.slime_necklace.passive_gen_desc")
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.fusion",
                        FUSION_COST_COUNT, FUSION_MANA_COST)
                .withStyle(ChatFormatting.BLUE));
        tooltip.add(Component.translatable("tooltip.slime_necklace.fusion_desc",
                        FUSION_INTERVAL / 20)
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.shield",
                        SHIELD_SLIME_COST, SHIELD_COOLDOWN / 20)
                .withStyle(ChatFormatting.LIGHT_PURPLE));
        tooltip.add(Component.translatable("tooltip.slime_necklace.shield_desc")
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.time_freeze",
                        String.format("%.0f", TIME_FREEZE_RADIUS),
                        TIME_FREEZE_DURATION / 20,
                        TIME_FREEZE_SLIME_COST)
                .withStyle(ChatFormatting.DARK_PURPLE));
        tooltip.add(Component.translatable("tooltip.slime_necklace.time_freeze_desc",
                        String.format("%.0f%%", TIME_FREEZE_HP_TRIGGER * 100),
                        TIME_FREEZE_COOLDOWN / 20)
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.reflect",
                        String.format("%.0f%%", REFLECT_DAMAGE_MULT * 100),
                        REFLECT_MANA_COST)
                .withStyle(ChatFormatting.DARK_AQUA));
        tooltip.add(Component.translatable("tooltip.slime_necklace.reflect_desc",
                        REFLECT_COOLDOWN / 20)
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.rain",
                        RAIN_PROJECTILE_COUNT, RAIN_SLIME_COST, RAIN_MANA_COST)
                .withStyle(ChatFormatting.RED));
        tooltip.add(Component.translatable("tooltip.slime_necklace.rain_desc",
                        RAIN_COOLDOWN / 20)
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("tooltip.slime_necklace.buff",
                        BUFF_THRESHOLD)
                .withStyle(ChatFormatting.GOLD));
        tooltip.add(Component.translatable("tooltip.slime_necklace.buff_desc")
                .withStyle(ChatFormatting.GRAY));
        ...
        if (tag.getBoolean(TAG_RAIN_ACTIVE)) {
            int fired = tag.getInt(TAG_RAIN_FIRED);
            tooltip.add(Component.translatable("tooltip.slime_necklace.rain_active",
                            fired, RAIN_PROJECTILE_COUNT)
                    .withStyle(ChatFormatting.RED, ChatFormatting.BOLD));
        }

        tooltip.add(Component.literal(""));
        tooltip.add(Component.translatable("tooltip.slime_necklace.hint_rain")
                .withStyle(ChatFormatting.DARK_GRAY));
        tooltip.add(Component.translatable("tooltip.bad_human")
    ...
    private void addCooldownLine(List<Component> tooltip, CompoundTag tag, String key, String translationKey) {
        int cd = tag.getInt(key);
        if (cd > 0) {
            tooltip.add(Component.translatable(translationKey + "_cd",
                            String.format("%.1f", cd / 20.0))
                    .withStyle(ChatFormatting.RED));
        } else {
            tooltip.add(Component.translatable(translationKey + "_ready")
                    .withStyle(ChatFormatting.GREEN));
        }
    }
