...
public class AquaSword extends SwordItem implements ManaItem, LensEffectItem {
    ...
    private static final String TAG_SOUND_PLAYED = "soundPlayed";
        ...
        if (player.isShiftKeyDown()) {
            if (!player.level().isClientSide) {
                ...
                player.displayClientMessage(
                        Component.translatable("message.ex_enigmaticlegacy.aqua_sword.power_level",
                                nextLevel + 1,
            ...
            for (LivingEntity living : entities) {
                if (!(living instanceof Player) ||
                    ...
                    if (!hasEnoughMana) {
                        if (!hasWaterSplash) {
                            player.displayClientMessage(
                                    Component.translatable("message.ex_enigmaticlegacy.aqua_sword.mana_required",
                                            manaCost),
                                    true);
                            hasWaterSplash = true;
                        }
...
//                        living.addEffect(new MobEffectInstance(ModEffects.DROWNING.get(), 2000, effectLevel));
    ...
    private boolean hasSoundPlayed(ItemStack stack) {
        return stack.getOrCreateTag().getBoolean(TAG_SOUND_PLAYED);
    }

    private void setSoundPlayed(ItemStack stack, boolean played) {
        stack.getOrCreateTag().putBoolean(TAG_SOUND_PLAYED, played);
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        ...
        list.add(Component.translatable("tooltip.ex_enigmaticlegacy.aqua_sword.power_level",
                powerLevel + 1));
        list.add(Component.translatable("tooltip.ex_enigmaticlegacy.aqua_sword.mana_cost",
                MANA_COSTS[powerLevel]));
        list.add(Component.translatable("tooltip.ex_enigmaticlegacy.aqua_sword.effect_level",
                EFFECT_LEVELS[powerLevel] + 1));
        list.add(Component.translatable("tooltip.ex_enigmaticlegacy.aqua_sword.internal_mana",
                internalMana, MAX_MANA));
        list.add(Component.translatable("tooltip.ex_enigmaticlegacy.aqua_sword.shift_hint"));
    }
    ...
    public void onUseTick(Level level, LivingEntity livingEntity, ItemStack stack, int count) {
        ...
        if (player.level().isClientSide) {
            ...
            if (!hasSoundPlayed(stack)) {
                playWaterSound(player);
                setSoundPlayed(stack, true);
            }
            ...
            for (int layer = 0; layer < 2; layer++) {
                ...
                for (int i = 0; i < wispCount; ++i) {
                    double angle = (i * tickIncrement + totalTime * 2) * Math.PI / 180.0;
                    double posX = player.getX() + Math.sin(angle) * radius;
                    double posY = baseHeight + heightOffset + Math.sin(time * 0.05 + i * 0.5) * 0.1;
            ...
            if (time % 4 == 0) {
                for (int i = 0; i < 4; i++) {
                    ...
                    WispParticleData topData = WispParticleData.wisp(
                            0.2F, 0.0F, 0.4F, 0.9F, 0.6F
                    );
                    player.level().addParticle(topData,
                            posX, player.getY() + 1.8, posZ,
                            0, -0.02F, 0);
            ...
            for (LivingEntity living : player.level().getEntitiesOfClass(LivingEntity.class, axis)) {
                if (!(living instanceof Player) ||
                    ...
                    if (hasEnoughMana && living.hurt(player.damageSources().playerAttack(player), 1.0F)) {
                        if (getInternalMana(stack) >= 15) {
