...
import org.xiaoyang.ex_enigmaticlegacy.Entity.others.EntitySeed;
...
import static vazkii.botania.api.block.FloatingFlower.IslandType.*;
...
public class SprawlRod extends Item {
    ...
    private boolean isGrassSeeds(ItemStack stack) {
        return stack.getItem() instanceof GrassSeedsItem ||
                (BotaniaItems.grassSeeds != null && stack.getItem() == BotaniaItems.grassSeeds);
    }
    ...
    public static Color getSeedColor(ItemStack seed) {
        if (seed.getItem() instanceof GrassSeedsItem) {
            GrassSeedsItem grassSeeds = (GrassSeedsItem) seed.getItem();
            if (grassSeeds instanceof FloatingFlowerVariant) {
