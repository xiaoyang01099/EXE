...
public class MagicTableRecipeCategory implements IRecipeCategory<MagicTableRecipe> {
    public static final ResourceLocation UID =
            new ResourceLocation(Exe.MODID, "magic_table");
    public static final RecipeType<MagicTableRecipe> RECIPE_TYPE =
            new RecipeType<>(UID, MagicTableRecipe.class);
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(Exe.MODID, "textures/gui/container/magic_table_gui.png");
    ...
    public MagicTableRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.title = Component.translatable(
                "gui." + Exe.MODID + ".category.magic_table");
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK,
                new ItemStack(ModBlocks.MAGIC_TABLE.get()));

        IDrawableStatic arrowStatic = guiHelper.drawableBuilder(
                        TEXTURE, ARROW_TEX_U, ARROW_TEX_V, ARROW_WIDTH, ARROW_HEIGHT)
                .setTextureSize(256, 256)
                .build();
        this.arrow = guiHelper.createAnimatedDrawable(
                arrowStatic, 60, IDrawableAnimated.StartDirection.LEFT, false);
    }
    ...
    @Override
    public void draw(@NotNull MagicTableRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull GuiGraphics guiGraphics, double mouseX, double mouseY) {
    ...
    @Override
    public @NotNull List<Component> getTooltipStrings(@NotNull MagicTableRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        if (mouseX >= ARROW_X && mouseX <= ARROW_X + ARROW_WIDTH
                && mouseY >= ARROW_Y && mouseY <= ARROW_Y + ARROW_HEIGHT) {
            return List.of(
                    Component.translatable(
                            "gui.ex_enigmaticlegacy.magic_table.tooltip.emc_cost",
                            recipe.getEmcCost().toString()
                    ).withStyle(ChatFormatting.GOLD),
                    Component.translatable(
                            "gui.ex_enigmaticlegacy.magic_table.tooltip.output",
                            formatCount(recipe.getOutputCount())
                    ).withStyle(ChatFormatting.AQUA),
                    Component.translatable(
                            "gui.ex_enigmaticlegacy.magic_table.tooltip.time",
                            recipe.getCraftTicks(),
                            String.format("%.1f", recipe.getCraftTicks() / 20.0)
