    ...
    private static void spawnBonusDrops(Level level, BlockPos pos, BlockState state) {
        ...
        if (state.is(BlockTags.LOGS) ||
            ...
            if (RANDOM.nextFloat() < 0.3f) {
                bonusDrops.add(new ItemStack(BotaniaItems.manaPearl, 1));
            }
        } else if (state.is(BlockTags.BASE_STONE_OVERWORLD)) {
            // 石头：掉落魔力粉
            if (RANDOM.nextFloat() < 0.2f) {
                bonusDrops.add(new ItemStack(BotaniaItems.manaPowder, RANDOM.nextInt(3) + 1));
            }
        } else if (isOre(state)) {
            // 矿石：更高概率掉落魔力钻石
            if (RANDOM.nextFloat() < 0.05f) {
                bonusDrops.add(new ItemStack(BotaniaItems.manaDiamond, 1));
            } else if (RANDOM.nextFloat() < 0.15f) {
                bonusDrops.add(new ItemStack(BotaniaItems.manaPearl, RANDOM.nextInt(2) + 1));
            }
        }
