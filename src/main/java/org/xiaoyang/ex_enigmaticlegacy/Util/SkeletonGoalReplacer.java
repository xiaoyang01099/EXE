...
import org.xiaoyang.ex_enigmaticlegacy.Entity.ai.WildHuntAwareGoal;
import org.xiaoyang.ex_enigmaticlegacy.Event.WildHuntArmorEventHandler;
    ...
    public static void onWorldTick(TickEvent.LevelTickEvent event) {
        if (event.phase == TickEvent.Phase.END && event.level instanceof ServerLevel) {
            if (event.level.getGameTime() % 10 == 0) {
                for (AbstractSkeleton skeleton : ((ServerLevel)event.level).getEntities(
