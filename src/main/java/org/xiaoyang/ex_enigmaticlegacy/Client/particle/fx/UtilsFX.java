package org.xiaoyang.ex_enigmaticlegacy.Client.particle.fx;
...
public class UtilsFX {
    ...
    public static void infusedStoneSparkle(Level world, int x, int y, int z, int md) {
        if (world.isClientSide) {
            ...
            for (int a = 0; a < 3; ++a) {
                FXSparkle fx = new FXSparkle((ClientLevel) world,
                        (double) ((float) x + world.random.nextFloat()),
                        (double) ((float) y + world.random.nextFloat()),
                        (double) ((float) z + world.random.nextFloat()),
                        1.75F, color == -1 ? world.random.nextInt(5) : color,
                        3 + world.random.nextInt(3));
                fx.setGravity(0.1F);
    ...
    public static Vec3 xCrossProduct(Vec3 v) {
        return new Vec3(0.0D, v.z, -v.y);
    }

    public static Vec3 zCrossProduct(Vec3 v) {
        return new Vec3(v.y, -v.x, 0.0D);
    }
        ...
        if (textureSizeCache.get(s) != null) {
            ...
            try {
                InputStream inputstream = Minecraft.getInstance()
                        .getResourceManager()
                        .getResource(new ResourceLocation("ex_enigmaticlegacy", s))
                        .orElseThrow()
                        .open();
    ...
    public static boolean isVisibleTo(float fov, Entity ent, double x, double y, double z) {
        ...
        if (dist < 4.0D) {
            ...
            double vT = (double)(fov + mc.options.fov().get() / 2.0F);
            int j = 512;
            if (j > 400) {
                j = 400;
            }
            ...
            return angle < vT && mc.options.getCameraType().isFirstPerson() && dist < rD
                    || !mc.options.getCameraType().isFirstPerson() && dist < rD;
        }
    }
    ...
    public static void drawFloatyGUILine(double x, double y, double x2, double y2, float partialTicks, int color, String texture, float speed, float distance) {
        ...
        poseStack.mulPose(Axis.ZP.rotationDegrees((float)(-(Math.atan2(d3, d4) * 180.0D / Math.PI)) + 90.0F));
        ...
        for (int i = 0; (float) i <= length; ++i) {
            ...
            float w = 1.0F;
            buffer.vertex(dx * (double) i, 0.0F - w, 0.0D).uv(f13 / w, f10).color(r, g, b, 1.0F).endVertex();
            buffer.vertex(dx * (double) i, 0.0F + w, 0.0D).uv(f14 / w, f9).color(r, g, b, 1.0F).endVertex();
        }
