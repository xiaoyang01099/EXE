...
public class FXScorch extends TextureSheetParticle {
    ...
    private void pushOutOfBlocks(double x, double y, double z) {
        ...
        if (this.level.getBlockState(blockpos).isSolidRender(this.level, blockpos)) {
            ...
            if (flag  && d0 < d3)        { d3 = d0;        b0 = 0; }
            if (flag1 && 1.0D - d0 < d3) { d3 = 1.0D - d0; b0 = 1; }
            if (flag2 && d1 < d3)        { d3 = d1;        b0 = 2; }
            if (flag3 && 1.0D - d1 < d3) { d3 = 1.0D - d1; b0 = 3; }
            if (flag4 && d2 < d3)        { d3 = d2;        b0 = 4; }
            if (flag5 && 1.0D - d2 < d3) { d3 = 1.0D - d2; b0 = 5; }

            float f  = this.random.nextFloat() * 0.05F + 0.025F;
            float f1 = (this.random.nextFloat() - this.random.nextFloat()) * 0.1F;

            switch (b0) {
                case 0: this.xd = -f;  this.yd = this.zd = f1; break;
                case 1: this.xd =  f;  this.yd = this.zd = f1; break;
                case 2: this.yd = -f;  this.xd = this.zd = f1; break;
                case 3: this.yd =  f;  this.xd = this.zd = f1; break;
                case 4: this.zd = -f;  this.xd = this.yd = f1; break;
                case 5: this.zd =  f;  this.xd = this.yd = f1; break;
            }
        }
    }
    ...
    public void setSize(float width, float height) {
        if (width != this.bbWidth || height != this.bbHeight) {
            ...
            this.setBoundingBox(new AABB(
                    d0, aabb.minY, d1,
                    d0 + (double)this.bbWidth,
                    aabb.minY + (double)this.bbHeight,
                    d1 + (double)this.bbWidth
            ));
        }
    }
}