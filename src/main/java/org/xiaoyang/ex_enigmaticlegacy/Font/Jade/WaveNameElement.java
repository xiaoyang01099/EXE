...
public class WaveNameElement extends Element {
    ...
    public void render(GuiGraphics guiGraphics, float x, float y, float maxX, float maxY) {
        ...
        switch (style) {
            ...
            case RAINBOW:
            case GLOW_STAR:
                ModRarities.drawWaveNameWithStyle(guiGraphics, font, stack, rawText, style,
                        (int) x, (int) y, alpha);
                break;
            case GLITCH:
                drawGlitchInJade(guiGraphics, font, rawText, x, y, alpha);
                break;
            case TEAR:
                drawTearInJade(guiGraphics, font, rawText, x, y, alpha);
                break;
            case DISSOLVE:
                drawDissolveInJade(guiGraphics, font, rawText, x, y, alpha);
                break;
            case SHATTER:
                drawShatterInJade(guiGraphics, font, rawText, x, y, alpha);
                break;
            default:
                guiGraphics.drawString(font, rawText, (int) x, (int) y, 0xFFFFFFFF, true);
                break;
        }
    ...
    private static float[] localToScreen(GuiGraphics guiGraphics, float localX, float localY) {
    ...
    private static void enableScissorFromLocal(GuiGraphics guiGraphics, float left, float top, float right, float bottom) {
        Window window = Minecraft.getInstance().getWindow();
        double guiScale = window.getGuiScale();
        int winH = window.getHeight();

        float[] screenLT = localToScreen(guiGraphics, left, top);
        float[] screenRB = localToScreen(guiGraphics, right, bottom);
    ...
    private static void drawChars(GuiGraphics guiGraphics, Font font, String raw, float startX, float startY, float offsetX, float offsetY, int color) {
        float cx = startX;
        for (int i = 0; i < raw.length(); i++) {
            ...
            guiGraphics.pose().translate(cx + offsetX, startY + offsetY, 0.0);
            guiGraphics.drawString(font, ch, 0, 0, color, true);
    ...
    private static void drawGlitchInJade(GuiGraphics guiGraphics, Font font, String raw, float startX, float startY, int alpha) {
        ...
        drawChars(guiGraphics, font, raw, drawX, drawY, 0, 0, baseColor);
        ...
        if (!isFlash) {
            ...
            for (int ch = 0; ch < 3; ch++) {
                ...
                enableScissorFromLocal(guiGraphics,
                        drawX - chromaRange - 2, drawY - 2,
                        drawX + totalWidth + chromaRange + 4, drawY + charHeight + 2);
                drawChars(guiGraphics, font, raw, drawX, drawY, chanOffX[ch], chanOffY[ch], cColor);
        ...
        for (int s = 0; s < sliceCount; s++) {
            ...
            drawChars(guiGraphics, font, raw, drawX, drawY, sliceOffsetX[s], 0, sliceColor);
            RenderSystem.disableScissor();
        }
        ...
        if (isBurst) {
            for (int s = 0; s < sliceCount; s++) {
                ...
                enableScissorFromLocal(guiGraphics, drawX, lineY, drawX + totalWidth, lineY + 1);
                drawChars(guiGraphics, font, raw, drawX, drawY, sliceOffsetX[s], 0, glowCol);
                RenderSystem.disableScissor();
            }
        }
    }

    private static void drawTearInJade(GuiGraphics guiGraphics, Font font, String raw, float startX, float startY, int alpha) {
        if (raw == null || raw.isEmpty()) return;
        ...
        for (int i = 0; i < raw.length(); i++) {
            ...
            for (int seg = 0; seg <= sliceCount; seg++) {
                ...
                guiGraphics.pose().pushPose();
                guiGraphics.pose().translate(curX + shift, startY, 0.0);
                guiGraphics.drawString(font, ch, 0, 0, segColor, true);
                guiGraphics.pose().popPose();

                if (seg == 0) {
                    int topColor = ((int) (alpha * 0.55f) << 24) | 0xFF4444;
                    guiGraphics.pose().pushPose();
                    guiGraphics.pose().translate(curX + shift - chromaX, startY - chromaY, 0.0);
                    guiGraphics.drawString(font, ch, 0, 0, topColor, true);
                    guiGraphics.pose().popPose();
                }
                ...
                if (seg == sliceCount) {
                    ...
                    guiGraphics.pose().pushPose();
                    guiGraphics.pose().translate(curX + shift + chromaX, startY + chromaY, 0.0);
                    guiGraphics.drawString(font, ch, 0, 0, botColor, true);
                    guiGraphics.pose().popPose();
                }
            ...
            for (int s = 0; s < sliceCount; s++) {
                ...
                if (GLITCH_RAND.nextFloat() < 0.80f) {
                    ...
                    guiGraphics.pose().pushPose();
                    guiGraphics.pose().translate(curX + glowShift, startY, 0.0);
                    guiGraphics.drawString(font, ch, 0, 0, glowColor, true);
                    guiGraphics.pose().popPose();
                    RenderSystem.disableScissor();
                }
            }
    ...
    private void drawDissolveInJade(GuiGraphics guiGraphics, Font font, String raw, float startX, float startY, int alpha) {
        if (raw == null || raw.isEmpty()) return;
        ...
        if (normalTop < normalBottom) {
            enableScissorFromLocal(guiGraphics, startX, normalTop, startX + totalW, normalBottom);
            float cx = startX;
            for (int i = 0; i < raw.length(); i++) {
                char c = raw.charAt(i);
                if (c == ' ') { cx += font.width(" "); continue; }
                String ch = String.valueOf(c);
                guiGraphics.pose().pushPose();
                guiGraphics.pose().translate(cx, startY, 0.0);
                guiGraphics.drawString(font, ch, 0, 0, (alpha << 24) | 0xFFFFFF, true);
                guiGraphics.pose().popPose();
                cx += font.width(ch);
            }
            RenderSystem.disableScissor();
        ...
        for (int col = 0; col < columnCount; col++) {
            ...
            for (int dy = 0; dy < stretchLen; dy++) {
                ...
                enableScissorFromLocal(guiGraphics, colX, sliceYtop, colX + colW, sliceYbot);
                ...
                for (int i = 0; i < raw.length(); i++) {
                    ...
                    if (cx + cw > colX && cx < colX + colW) {
                        guiGraphics.pose().pushPose();
                        guiGraphics.pose().translate(cx, startY, 0.0);
                        guiGraphics.drawString(font, String.valueOf(c), 0, 0, dropColor, true);
                        guiGraphics.pose().popPose();
                    }
                    cx += cw;
                }
        ...
        if (dissolveY > 0 && dissolveY <= charH + 2) {
            ...
            enableScissorFromLocal(guiGraphics, startX, surfY, startX + totalW, surfY + 1.5f);
    ...
    private static void drawShatterInJade(GuiGraphics guiGraphics, Font font, String raw, float startX, float startY, int alpha) {
        if (raw == null || raw.isEmpty()) return;
    ...
    private static void drawIntactChar(GuiGraphics guiGraphics, Font font, char c,
                                       float x, float y, int alpha, long ms, int idx) {
        ...
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(x, y + jitterY, 0);
        guiGraphics.drawString(font, String.valueOf(c), 0, 0, argb, true);
        guiGraphics.pose().popPose();
    }

    private static void drawShardTriangleJade(GuiGraphics guiGraphics, Font font, String ch, float charX, float charY, ShardData sd, float crackT, float scatterT, int alpha, long ms, int charIdx, boolean isReform) {
        ...
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(charX + sd.cx + dispX, charY + sd.cy + dispY, 0);
        if (Math.abs(rot) > 0.001f) {
            guiGraphics.pose().mulPose(new Quaternionf().rotateZ(rot));
        }
        guiGraphics.pose().translate(-sd.cx, -sd.cy, 0);
        guiGraphics.drawString(font, ch, 0, 0, shardColor, true);
        guiGraphics.pose().popPose();
        RenderSystem.disableScissor();

        if (!isReform && scatterT > 0.1f && scatterT < 0.9f) {
            drawShardTrailJade(guiGraphics, font, charX, charY, sd,
                    scatterT, alpha, ms);
        }
    }

    private static void drawShardTrailJade(GuiGraphics guiGraphics, Font font, float charX, float charY, ShardData sd, float scatterT, int alpha, long ms) {
        int trailCount = 3;
        for (int t = 1; t <= trailCount; t++) {
            ...
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(tx - 1f, ty - 1f, 0);
            guiGraphics.drawString(font, "·", 0, 0, color, true);
            guiGraphics.pose().popPose();
        }
    }

    private static void drawCrackLinesJade(GuiGraphics guiGraphics, Font font, String ch, float charX, float charY, ShardData[] shards, float crackT, int alpha, long ms) {
        ...
        for (ShardData sd : shards) {
            ...
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(px - 1f, py - 1f, 0);
            guiGraphics.drawString(font, "✦", 0, 0, ptColor, true);
            guiGraphics.pose().popPose();
        }
    }
}