...
const vec2 LIGHT_EFFECT_NOISE1_TILE = vec2(0.5, 0.5); // Noise1 的 X/Y 平铺倍率，只影响 LIGHT_EFFECT shader。
const vec2 LIGHT_EFFECT_NOISE2_TILE = vec2(0.5, 0.5); // Noise2 的 X/Y 平铺倍率，只影响 LIGHT_EFFECT shader。
...
// 统一裁剪 LIGHT_EFFECT 的最终可见区域和 Bloom source，避免三噪声能量露出方形边。
...
void main() {
    ...
    float roundMask = circleMask(vBloomTexCoord, maskRadius, maskSoftness);
