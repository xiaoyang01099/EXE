...
void main() {
    // gl_InstanceID 现在是 active list 内的位置，不再直接等于 Particle SSBO 下标。
    ...
    // active index 已经按 pipeline 压缩，这里的检查只保留为异常数据兜底。
    ...
    // 从 view 矩阵提取相机右/上方向，生成始终朝向相机的 billboard。
    ...
    vShapeSeed = vec3(p.extra.y, p.extra.z, p.origin.w);
    vShapeType = int(p.render.w + 0.5);
}
